...
import { accuracy, phaseAccuracy } from './accuracy';
...
export class LocalAnalysisEngine {
  ...
  private review() {
    const division = { ...this.ctrl.data.game.division };
    const { totals, infos } = annotate(this.nodes);
    const { white, black } = accuracy(this.nodes);
    const phases = phaseAccuracy(this.nodes, division);
    const nodesPerMove =
    ...
    return {
      localAnalysis: {
        ...
        analysis: {
          ...
          white: { ...totals.white, accuracy: white.percent, acpl: white.acpl, phases: phases.white },
          black: { ...totals.black, accuracy: black.percent, acpl: black.acpl, phases: phases.black },
          phases,
          partial: false,
        },
        engine,
