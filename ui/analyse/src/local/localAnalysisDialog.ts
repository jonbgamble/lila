...
class LocalAnalysisDialog {
  ...
  currentAnalysisHtml() {
    const info = this.ctrl.staticAnalysis?.engine;
    if (!info) return '';
    const versionFields = info.engineVersion.split('/');
    const engine =
      versionFields.length === 3 ? `fishnet: ${versionFields[1]}` : `browser: ${info.engineVersion}`;
    ...
    const clearButton = this.ctrl.idbTree.hasLocalAnalysis
      ? `<a class="clear-local"><span class="svg-icon icon-x"></span></a>`
      : '';
    return $html`
  ...
  getMode(val: Preset) {
    ...
    const id = this.ctrl.ceval.engines.supporting(this.ctrl.variantKey, 'staticAnalysis', 'browser')[0]?.id;
