...
import { canPublishAnalysis, LocalAnalysisEngine, uploadAnalysis } from './localAnalysisEngine';
...
class LocalAnalysisDialog {
    ...
    return [
      ...
      <span class="footer">
        ...
        <button
          ...
          on={{
            click: async () => {
              if (await this.clickPublish(dialog)) redraw();
            },
          }}>
  ...
  private async clickPublish(dialog: Dialog): Promise<boolean> {
    if (this.canUpload.whyNot) {
      await alert(this.canUpload.whyNot);
      return false;
    }
    if (
      this.publishedNpm &&
